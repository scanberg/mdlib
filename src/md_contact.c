...
bool md_contact_pairs_for_each(const md_contact_pairs_t* p, const md_system_state_t* state, md_contact_pair_callback_t callback, void* user_param) {
    ...
    if (state->num_atoms != p->num_atoms || (p->num_atoms && !(state->x && state->y && state->z))) {
    ...
    if (self) {
        md_coord_stream_t coords = md_coord_stream_from_soa(state->x, state->y, state->z, p->a_atoms, p->num_a_atoms);
        ...
        md_coord_stream_t internal = md_coord_stream_from_soa(state->x, state->y, state->z, p->b_atoms, p->num_b_atoms);
        md_coord_stream_t external = md_coord_stream_from_soa(state->x, state->y, state->z, p->a_atoms, p->num_a_atoms);
