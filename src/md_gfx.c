...
md_gfx_handle_t md_gfx_structure_create_from_mol(const md_system_t* mol) {
    ASSERT(mol);
    return md_gfx_structure_create((uint32_t)mol->atom.count, (uint32_t)md_array_size(mol->bond.pairs), (uint32_t)mol->protein_backbone.segment.count, (uint32_t)mol->protein_backbone.range.count, (uint32_t)mol->comp.count, (uint32_t)mol->assembly.count);
}
