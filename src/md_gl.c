...
static bool draw_licorice  (gl_program_t program, const molecule_t* mol, gl_buffer_t atom_color, float radius, float max_length, md_gl_licorice_mode_t mode, uint32_t uniform_color);
...
bool md_gl_draw(const md_gl_draw_args_t* args) {
    ...
    for (size_t i = 0; i < md_array_size(draw_ops); i++) {
        ...
        case MD_GL_REP_LICORICE:
            draw_licorice(shaders->licorice[program_permutation],    mol, rep->atom_color, 0.2f * scale * draw_op->args.licorice.radius, max_length, draw_op->args.licorice.color_mode, draw_op->args.licorice.uniform_color);
            break;
        case MD_GL_REP_BALL_AND_STICK:
            draw_licorice(shaders->licorice[program_permutation],    mol, rep->atom_color, 0.2f * scale * draw_op->args.ball_and_stick.stick_radius, max_length, draw_op->args.ball_and_stick.color_mode, draw_op->args.ball_and_stick.uniform_color);
...
static bool draw_licorice(gl_program_t program, const molecule_t* mol, gl_buffer_t atom_color, float radius, float max_length, md_gl_licorice_mode_t mode, uint32_t uniform_color) {
	...
	} params = { radius, max_length * max_length, mode, uniform_color };
