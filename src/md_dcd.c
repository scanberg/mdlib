...
typedef struct dcd_t {
    uint64_t magic;
    str_t    filepath;           // Retained so each load_frame can open its own file handle
...
static bool dcd_parse_file_header(md_file_o* file, dcd_file_header_t* out, md_allocator_i* alloc) {
    ASSERT(file && out && alloc);
...
static int dcd_read_coord_record(md_file_o* file, int expected_count, bool rev, float* out) {
    int32_t sz = 0;
    if (md_file_read(file, &sz, 4) != 4) return -1;
    ...
    if (out) {
        if (md_file_read(file, out, (size_t)sz) != (size_t)sz) return -1;
        dcd_swap_float_array(out, n, rev);
    } else {
        if (!md_file_seek(file, sz, MD_FILE_CUR)) return -1;
    }

    int32_t sz2 = 0;
    if (md_file_read(file, &sz2, 4) != 4) return -1;
...
static bool dcd_read_frame_at(md_file_o* file,
                               ...
                               float* out_x, float* out_y, float* out_z)
{
    ...
    if (charmm & DCD_HAS_EXTRA_BLOCK) {
        ...
        if (block_sz == 48) {
            ...
            if (!md_file_seek(file, block_sz, MD_FILE_CUR)) {
                MD_LOG_ERROR("DCD: Failed to skip unknown extra block");
                return false;
            }
        }

        int32_t block_sz2 = 0;
        if (md_file_read(file, &block_sz2, 4) != 4) {
    ...
    if (all_atoms) {
        ...
        if (dcd_read_coord_record(file, natoms, rev, coords_requested ? out_x : NULL) < 0) {
            MD_LOG_ERROR("DCD: Failed to read X coordinate record");
            return false;
        }
        if (dcd_read_coord_record(file, natoms, rev, coords_requested ? out_y : NULL) < 0) {
            MD_LOG_ERROR("DCD: Failed to read Y coordinate record");
            return false;
        }
        if (dcd_read_coord_record(file, natoms, rev, coords_requested ? out_z : NULL) < 0) {
        ...
        if (coords_requested) {
            ...
            for (int dim = 0; dim < 3; ++dim) {
                ...
                if (dcd_read_coord_record(file, nfree, rev, tmp) < 0) {
            ...
            for (int dim = 0; dim < 3; ++dim) {
                if (dcd_read_coord_record(file, nfree, rev, NULL) < 0) {
    ...
    if (charmm & DCD_HAS_4DIMS) {
        if (dcd_read_coord_record(file, atoms_in_frame, rev, NULL) < 0) {
...
static bool dcd_load_frame(struct md_trajectory_o* inst, int64_t idx, md_trajectory_frame_header_t* out_hdr, float* x, float* y, float* z) {
	...
	md_file_o* file = md_file_open(dcd->filepath, MD_FILE_READ | MD_FILE_BINARY);
	if (!file) {
		MD_LOG_ERROR("DCD: Failed to open '" STR_FMT "' for frame loading", STR_ARG(dcd->filepath));
		return false;
	}
	const int64_t offset = dcd_frame_offset(dcd, idx);
	if (!md_file_seek(file, offset, MD_FILE_BEG)) {
		MD_LOG_ERROR("DCD: Failed to seek to frame %lld at offset %lld", idx, offset);
		md_file_close(file);
		return false;
	}
	...
	bool success = dcd_read_frame_at(file, natoms, dcd->header.nfixed, dcd->header.charmm, dcd->header.reverse_endian,
...
md_trajectory_i* md_dcd_trajectory_create(str_t filename, md_allocator_i* ext_alloc, md_trajectory_flags_t flags) {
    ...
    if (fhdr.nfixed > 0) {
		...
		int64_t first_frame_offset = dcd_frame_offset(dcd, 0);
        if (!md_file_seek(file, first_frame_offset, MD_FILE_BEG)) {
            MD_LOG_ERROR("DCD: Failed to seek to first frame for fixed-atom initialisation");
            goto fail;
        }
        // Read the first frame treating nfixed as 0 so that all atoms are read.
        if (!dcd_read_frame_at(file, natoms, 0, fhdr.charmm, fhdr.reverse_endian,
                               NULL, true, NULL,
