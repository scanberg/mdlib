...
enum {
    ...
    MD_UTIL_POSTPROCESS_HBOND_BIT           = 0x0200,
    MD_UTIL_POSTPROCESS_ORDER_BIT           = 0x0400,
...
void md_util_covalent_bonds_compute_exp(md_bond_data_t* out_bonds, const float* in_x, const float* in_y, const float* in_z, const md_element_t* in_elem, size_t atom_count, const md_residue_data_t* in_res, const md_unitcell_t* in_cell, struct md_allocator_i* alloc);
...
static inline void md_util_covalent_bonds_compute(md_bond_data_t* out_bonds, const md_molecule_t* mol, struct md_allocator_i* alloc) {
    md_util_covalent_bonds_compute_exp(out_bonds, mol->atom.x, mol->atom.y, mol->atom.z, mol->atom.element, mol->atom.count, &mol->residue, &mol->unitcell, alloc);    
}
