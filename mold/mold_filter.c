...
/*
uint32_t lex_expression(mem_arena_t* mem_arena, const char* str, uint32_t len) {
    int balance = 0;
    ...
    for (uint32_t i = 0; i < len;) {
        if (is_whitespace(str[i])) {
            ...
            token_t* t = (token_t*)arena_alloc(mem_arena, sizeof(token_t));
            if (!t) {
                log_error("Out of memory");
                return 0;
            }
            t->str.offset = (uint16_t)i;

            bool is_float = false;
            while (i < len && !is_whitespace(str[i]) && str[i] != ',' && str[i] != ')' && str[i] != ':') {
                if (is_digit(str[i])) {
                }
                else if (str[i] == '.' && !is_float) {
                    is_float = true;
                }
                else {
                    t->str.length = (uint16_t)i - t->str.offset;
                    log_error_with_context("Invalid character found in numeric literal", str, len, t->str);
                    return 0;
                }
                ++i;
            }
            ...
            if (is_float) {
                t->type = TOKEN_FLT;
                t->data._flt = parse_float(str + t->str.offset, t->str.length);
            }
            else {
                t->type = TOKEN_INT;
                t->data._int = parse_int(str + t->str.offset, t->str.length);
            }
            ++tok_count;
        }
            ...
            t->type = TOKEN_UNDEFINED;
            t->str.offset = (uint16_t)i;
            while (i < len && (is_alpha(str[i]) || is_digit(str[i]) || str[i] == '_')) ++i;
            t->str.length = (uint16_t)i - t->str.offset;

            uint32_t idx;
            if (idx = match_keyword(str + t->str.offset, t->str.length)) {
                t->type = keyword_table[idx].type;
            } else if (idx = match_ident_str(str + t->str.offset, t->str.length)) {
                t->type = TOKEN_IDENT;
            } else {
                while (i < len && is_alpha(str[i]) && is_digit(str[i])) ++i;
                t->str.length = (uint16_t)i - t->str.offset;
                t->type = TOKEN_STR;
            }
...
void mold_filter_molecule_extract(mold_filter_molecule* filt_mol, const mold_molecule* mol) {
    ...
    filt_mol->atom.count     = mol->atom.count;
    filt_mol->atom.x         = mol->atom.x;
    filt_mol->atom.y         = mol->atom.y;
    filt_mol->atom.z         = mol->atom.z;
    filt_mol->atom.radius    = mol->atom.radius;
    filt_mol->atom.mass      = mol->atom.mass;
    filt_mol->atom.element   = mol->atom.element;
    filt_mol->atom.name      = mol->atom.name;
    filt_mol->atom.bfactor   = mol->atom.bfactor;
    filt_mol->atom.occupancy = mol->atom.occupancy;
    filt_mol->atom.bond      = mol->atom.bond;
    ...
    filt_mol->chain.count      = mol->chain.count;
    filt_mol->chain.id         = mol->chain.id;
    filt_mol->chain.atom_range = mol->chain.atom_range;
...
mold_filter* mold_filter_create(const char* expr_str, uint32_t expr_len) {
    char tok_mem[1 << 12];
    mem_arena_t tok_arena = { 0 };
    arena_init(&tok_arena, tok_mem, ARRAY_SIZE(tok_mem));

    const uint32_t num_tokens = lex_expression(&tok_arena, expr_str, expr_len);
    const token_t* tokens = (const token_t*)tok_arena.buf;

#if 0
    printf("Done lexing tokens.. %u found\n", num_tokens);
    for (uint32_t i = 0; i < num_tokens; ++i) {
        printf("%.*s\n", (int)(tokens[i].str.end - tokens[i].str.beg), tokens[i].str.beg);
    }
#endif
...
#endif
            const size_t expr_offset = sizeof(mold_filter);
            const size_t tree_offset = expr_offset + (expr_len + 1);
            const size_t mem_size = tree_offset + ast_arena.offset;
            mold_filter* filt = (mold_filter*)calloc(mem_size, 1);
            memcpy((uint8_t*)filt + expr_offset, expr_str, expr_len);
            memcpy((uint8_t*)filt + tree_offset, ast_arena.buf, ast_arena.offset);
            filt->expr_str = (const char*)((uint8_t*)filt + expr_offset);
            filt->expr_len = expr_len;
            filt->root_offset = (uint32_t)(tree_offset + ((uintptr_t)root - (uintptr_t)ast_arena.buf));
            return filt;
        }
    }
...
void mold_filter_destroy(mold_filter* filt) {
    assert(filt);
    free(filt);
}
*/
