...
md_gfx_handle_t md_gfx_structure_create_from_mol(const md_molecule_t* mol) {
    ASSERT(mol);
    return md_gfx_structure_create((uint32_t)mol->atom.count, (uint32_t)md_array_size(mol->bonds), (uint32_t)mol->backbone.count, (uint32_t)mol->backbone.range_count, (uint32_t)mol->residue.count, (uint32_t)mol->instance.count);
}
