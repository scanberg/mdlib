...
md_gfx_handle_t md_gfx_structure_create_from_mol(const md_system_t* mol) {
    ASSERT(mol);
    return md_gfx_structure_create((uint32_t)mol->atom.count, (uint32_t)md_array_size(mol->bond.pairs), (uint32_t)mol->protein_backbone.segment.count, (uint32_t)mol->protein_backbone.range.count, (uint32_t)mol->component.count, (uint32_t)mol->assembly.count);
}
...
void recompute_clusters2(structure_t* s, const float* x, const float* y, const float* z, uint32_t count, uint32_t byte_stride) {
    ...
    md_util_sort_spatial_xyz(grp_src_indices, grp_cent, sizeof(vec3_t), grp_count);
