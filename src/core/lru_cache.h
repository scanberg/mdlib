...
// This is an 8-way associative cache of size N, meaning that the supplied pointer will be binned into cache blocks with 8 fully associative lines.
// 
typedef struct lru_cache lru_cache;
struct lru_cache;

void lru_cache_init(lru_cache* cache, uint32_t num_lines);
void lru_cache_free(lru_cache* cache);

void* lru_cache_get(lru_cache* cache, int idx);
void  lru_cache_put(lru_cache* cache, void* ptr);
